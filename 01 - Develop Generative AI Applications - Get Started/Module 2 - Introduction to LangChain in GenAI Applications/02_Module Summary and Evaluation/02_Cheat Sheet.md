...
> Source table reconstructed from raw pasted text (the original was a "Package/Method / Description / Code Example" table; formatting — including curly quotes and `//` comments — has been cleaned up, with code corrected to valid Python where the source's smart-quote mangling broke it).
...
A document loader from `langchain_community.document_loaders` that loads PDFs into `Document` objects. You can use this document loader to extract text content from PDF files.
...
A document loader from `langchain_community.document_loaders` that loads content from websites into `Document` objects. You can use this document loader to extract text content from web pages.
...
A lightweight wrapper from `langchain.memory` that provides convenient methods for saving `HumanMessage`s, `AIMessage`s, and then fetching them all. You can use the `ChatMessageHistory` wrapper to maintain conversation history.
...
A memory module from `langchain.memory` that allows for the storage of messages and conversation history. You can use this memory module with conversation chains to maintain context across multiple interactions.
...
A decorator from `langchain.tools` that simplifies the creation of custom tools. This decorator automatically converts a function into a `Tool` object.
...
> **Note:** The source page's cheat sheet table cuts off here — no code example or further description was given for `AgentExecutor` beyond this one description sentence. Not filled in, since the source doesn't provide it.

...
- **Audio Notes** — one narrated MP3 per lesson, in each module's `Audio Notes/` folder, synthesized from that lesson's markdown notes with Kokoro-82M (`af_heart` voice). Course 1 is fully covered — 9 lessons, ~3 hours of narration. Code-heavy companion files (the "Understanding Coding" walkthroughs) are skipped, since code doesn't narrate usefully.

> The MP3s are stored with **[Git LFS](https://git-lfs.com)**, so cloning needs `git lfs install` first (otherwise you get small pointer files instead of audio). The lecture `.mp4` files are deliberately *not* tracked — they're IBM/Coursera copyrighted material, and three of them exceed GitHub's 100 MB per-file limit. Their content lives here as transcripts and Video Notes instead.
